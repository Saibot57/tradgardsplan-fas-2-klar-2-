 ...
 */
import { sampleSunHourly, projectShadow, rectCorners, DEFAULT_LOCATION, } from "@kolonitradgard/spatial-core";
const WALL = {
    id: "south-wall",
    cx: 5000,
    cy: 3000,
...
function main() {
    ...
    console.log(`Solar prototype — Landskrona midsummer (${midsummer.toDateString()})\n`);
    console.log("time   altitude°  azimuth°   shadow length (mm)   bed-A shaded?");
    console.log("------------------------------------------------------------------");
    let sunHours = 0;
    for (const { date, sun } of samples) {
        ...
        let shadedFrac = 0;
        if (shadow)
            shadedFrac = bedShadedFraction(BED, shadow);
        const lit = shadedFrac < 0.5;
        if (lit && sun.altitudeRad > 0)
            sunHours++;
        console.log(`${h}:${m}    ${altDeg}    ${azDeg}            ${len}    ${shadow ? `${(shadedFrac * 100).toFixed(0)}%` : "—"} ${lit ? "(sun)" : "(shade)"}`);
    }
    console.log(`\nUppskattat antal soltimmar för bed-A vid midsommar: ~${sunHours} h`);
    console.log("(Sampling 1/h → upplösning ±30 min, se docs/product_scope.md)");
}
main();
