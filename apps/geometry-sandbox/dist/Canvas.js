...
import { computeGhostRect } from "./ghostRect.js";
 ...
 */
function pickRectStyle(rect, isWall, palette) {
    if (rect.color) {
...
function drawDashedLineWithLabel(ctx, pA, pB, label, vp, strokeStyle, textStyle) {
 ...
 */
function drawMeasurementOverlay(ctx, primary, allRects, boundary, vp, showAll, palette) {
    // 1. Grannmått
    const neighbors = computeNeighborMeasurements(primary, allRects, NEIGHBOR_DISTANCE_THRESHOLD_MM, showAll);
    for (const n of neighbors) {
        const label = `${Math.round(n.distanceMm)} mm`;
        drawDashedLineWithLabel(ctx, n.pA, n.pB, label, vp, palette.ink2, palette.measurementText);
    }
    ...
    for (const e of edges) {
        ...
        drawDashedLineWithLabel(ctx, wFrom, wTo, label, vp, palette.accentSun, palette.measurementText);
    }
}
export function Canvas({ state, dispatch, sun, overlappingIds, touchingIds, theme }) {
    ...
    useEffect(() => {
        ...
        if (state.showSunPath) {
            ...
            for (const sample of samples) {
                if (sample.sun.altitudeRad <= 0)
                    continue;
                for (const rect of state.rectangles) {
                    if (rect.wallHeight <= 0)
                        continue;
        ...
        for (const rect of state.rectangles) {
            ...
            ctx.fillText(`${rect.id}  ${rect.width}×${rect.height} mm  ${rect.rotationDeg.toFixed(0)}°${isWall ? `  H=${rect.wallHeight}` : ""}`, center.x + 6, center.y - 6);
        }
        ...
        if (drag && primaryRect && (drag.mode.kind === "move" || drag.mode.kind === "resize")) {
            drawMeasurementOverlay(ctx, primaryRect, state.rectangles, state.plot.boundaryRect, state.viewport, state.showAllMeasurements, palette);
            ...
            if (drag.mode.kind === "move") {
                ...
                ctx.fillText(`Δ ${dx >= 0 ? "+" : ""}${dx} , ${dy >= 0 ? "+" : ""}${dy} mm`, c.x + 12, c.y - 4);
            }
        }
        ...
        if (ghostRect && dragRef.current?.mode.kind === "creating") {
            ...
            ctx.fillStyle = palette.measurementText;
            ctx.font = "600 13px var(--font-mono, ui-monospace, Menlo, monospace)";
            ctx.fillText(`${Math.round(ghostRect.width)} × ${Math.round(ghostRect.height)} mm`, center.x + 12, center.y - 12);
        }
...
function drawPlotBoundary(ctx, rect, vp, palette) {
    const corners = rectCorners(rect);
...
function drawCompass(ctx, w, palette, northRotationDeg = 0) {
    const cx = w - 50;
    const cy = 50;
    const r = 24;
    ctx.save();
