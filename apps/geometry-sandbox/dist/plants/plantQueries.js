 ...
 */
import { parseSunCategory } from "./format.js";
 ...
 * (match everything).
 */
export function matchesQuery(plant, query) {
    const needle = query.trim().toLowerCase();
    if (needle.length < MIN_QUERY_LENGTH)
        return true;
    if (plant.commonName.toLowerCase().includes(needle))
        return true;
    if (plant.commonNameEn && plant.commonNameEn.toLowerCase().includes(needle))
        return true;
    if (plant.scientificName.toLowerCase().includes(needle))
        return true;
    return false;
}
export function matchesCategory(plant, category) {
    if (category === "all")
        return true;
    return plant.category === category;
}
export function matchesSun(plant, sun) {
    if (sun === "all")
        return true;
    return parseSunCategory(plant.sunCategory).includes(sun);
}
export function applyPlantFilters(plants, filters) {
    const query = filters.query ?? "";
    const category = filters.category ?? "all";
    const sun = filters.sun ?? "all";
    return plants.filter((p) => matchesQuery(p, query) && matchesCategory(p, category) && matchesSun(p, sun));
}
const SV_COLLATOR = new Intl.Collator("sv");
/** Stable copy sorted by Swedish locale comparison of `commonName`. */
export function sortByCommonName(plants) {
    return [...plants].sort((a, b) => SV_COLLATOR.compare(a.commonName, b.commonName));
}
//# sourceMappingURL=plantQueries.js.map