...
export function fmtTemp(n) {
    return `${fmtNum(n, 0)} °C`;
}
export function fmtPct(n) {
    return `${fmtNum(n, 0)} %`;
}
export function fmtMicroS(n) {
 ...
 *   > 1600     → "Hög näring"
 */
export function ecLabel(maxMicroS) {
    if (maxMicroS < 800)
        return "Låg näring";
    if (maxMicroS <= 1600)
        return "Medelhög näring";
    return "Hög näring";
}
/**
 ...
 */
export function parseSunCategory(s) {
    if (!s)
        return [];
    const seen = new Set();
    const out = [];
    for (const part of s.split("/")) {
        ...
        let token = null;
        if (p === "full sol" || p === "full sun")
            token = "full";
        else if (p === "halvskugga" || p === "partial shade" || p === "partial")
            token = "partial";
        else if (p === "skugga" || p === "shade")
            token = "shade";
...
export function sunCategoryLabel(s) {
    if (!s)
        return "—";
    return s;
}
