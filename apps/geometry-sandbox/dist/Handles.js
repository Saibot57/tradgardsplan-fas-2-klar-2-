 ...
 */
import { localToWorld, rectCorners, worldToLocal, worldToScreen, } from "@kolonitradgard/spatial-core";
...
export function hitTestHandle(screenPoint, rect, vp) {
    ...
    for (const { id, screen } of positions) {
        if (id === "rotate") {
            ...
            if (d <= ROTATE_HANDLE_RADIUS_PX + 2)
                return id;
        }
    }
    for (const { id, screen } of positions) {
        ...
        if (Math.abs(screen.x - screenPoint.x) <= half &&
 ...
 */
export function computeResizedRect(oldRect, handle, pointerWorld) {
    ...
    if (sx !== 0)
        newW = Math.max(MIN_RECT_DIMENSION_MM, sx * 2 * localPointer.x);
    if (sy !== 0)
        newH = Math.max(MIN_RECT_DIMENSION_MM, sy * 2 * localPointer.y);
    // Anchor in oldRect's local space: opposite of the handle.
    const anchorLocalOld = {
    ...
    const anchorLocalNew = { x: (-sx * newW) / 2, y: (-sy * newH) / 2 };
 ...
 */
export function computeRotation(rect, pointerWorld) {
...
export function drawHandles(ctx, rect, vp, palette) {
    const positions = getHandleScreenPositions(rect, vp);
